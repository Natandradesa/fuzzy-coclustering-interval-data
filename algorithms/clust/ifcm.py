...
class IFCM(BaseFuzzy):
    ...
    References
    ----------
    .. [1] de Carvalho, F. A. T. (2007).
           "Fuzzy c-means clustering methods for symbolic interval data."
           Pattern Recognition Letters, 28(4), 423-437.
           https://doi.org/10.1016/j.patrec.2006.08.014
    """
    ...
    def fit(self, X, random_state=0):
        """
        Fit the IFCM model to an interval-valued dataset.
        ...
        if self.variable_names is not None:
            self.prototypes_ = pd.DataFrame(
                self.prototypes_, columns=self.variable_names
            )
    ...
    def _fit_single(self, X_lower, X_upper, random_state):
        ...
        for it in range(1, self.max_iter + 1):
            ...
            Dist = cdist(XA=X_lower, XB=C_lower, metric="sqeuclidean") + cdist(
