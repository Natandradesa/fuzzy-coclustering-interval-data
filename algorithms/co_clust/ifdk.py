...
class IFDK:
    """
    Interval Fuzzy Double K-means algorithm.
    ...
    def fit(self, X, random_state=0):
        """
        Fit the IFDK model to an interval-valued dataset.
    ...
    def _update_prototypes(
        self, X_lower, X_upper, G_raised_m, H_raised_n, C_lower, C_upper
    ):
        """
        ...
        for o in range(self.n_sample_clusters):
            ...
            for v in range(self.n_feature_clusters):
                h_v = H_raised_n[:, v]
                denom = g_o.sum() * h_v.sum()
                if denom > self.ld:
                    C_lower_new[o, v] = (g_o @ X_lower @ h_v) / denom
                    C_upper_new[o, v] = (g_o @ X_upper @ h_v) / denom
                else:
    ...
    def _fit_single(self, X_lower, X_upper, random_state):
        ...
        # random prototypes, used only in case of indeterminatiions at the first iteration.
        ...
        for it in range(1, self.max_iter + 1):
            ...
            dist_array = squared_distances(
                X_lower=X_lower, X_upper=X_upper, C_lower=C_lower, C_upper=C_upper
            )
